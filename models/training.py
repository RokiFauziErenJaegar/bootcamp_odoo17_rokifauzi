...
class TrainingCourse(models.Model):
    ...
    level = fields.Selection([('basic', 'Dasar'), ('advanced', 'Lanjutan')], string='Tingkatan',
    default='basic')
...
class TrainingSession(models.Model):
    ...
    course_id = fields.Many2one('training.course', string='Judul Kursus', required=True,
    ondelete='cascade')
    ...
    end_date = fields.Date(string="Tanggal Selesai", compute='get_end_date',
    inverse='set_end_date', store=True)
    attendees_count = fields.Integer(string="Jumlah Peserta", compute='get_attendees_count',
    store=True)
...
class TrainingAttendee(models.Model):
    ...
    # name = fields.Char(string='Nama', required=True) # atau dihapus saja
    #name = fields.Char(string='Nama', required=True)
    gender = fields.Selection([('male', 'Pria'), ('female', 'Wanita')], string='Gender', required=True, help='Jenis Kelamin')
    marital = fields.Selection([
    ('single', 'Belum Menikah'),
    ('married', 'Menikah'),
    ('divorced', 'Cerai')], string='Pernikahan', help='Status Pernikahan')
